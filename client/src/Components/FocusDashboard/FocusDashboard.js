  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [statsRes, trendRes, tasksRes] = await Promise.all([
        fetch('http://localhost:5000/api/focus/stats', { headers: authHeaders }),
        fetch('http://localhost:5000/api/focus/productivity-trend', { headers: authHeaders }),
        fetch('http://localhost:5000/api/tasks/all', { headers: authHeaders }),
      ]);
